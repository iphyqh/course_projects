...
static Obj* getConstAttenField(Obj* obj)
{
	if (obj == NULL) {
		throw ParseError(string("Object contains no Const Atten field"));
	}

	const dict& d = obj->getDict();
	dict::const_iterator i;
	if ((i = d.find("constant_attenuation_coeff")) != d.end()) {
		return (*i).second;
	}

	throw ParseError(string("Object contains no Const Atten field"));
}
static Obj* getLinearAttenField(Obj* obj)
{
	if (obj == NULL) {
		throw ParseError(string("Object contains no linear Atten field"));
	}

	const dict& d = obj->getDict();
	dict::const_iterator i;
	if ((i = d.find("quadratic_attenuation_coeff")) != d.end()) {
		return (*i).second;
	}

	throw ParseError(string("Object contains no Quadric Atten field"));
}

static Obj* getQuadricAttenField(Obj* obj)
{
	if (obj == NULL) {
		throw ParseError(string("Object contains no Const Atten field"));
	}
...
static double ToDouble(Obj* obj)
{
	return obj->getScalar();
}
...
static void processObject( Obj *obj, Scene *scene, mmap& materials )
{
	...
	if( name == "directional_light" ) {
		...
		double constantAttenuation = 0.0;
		double linearAttenuation = 0.0;
		double quadricAttenuation = 0.0;

		if (hasField(child, "constant_attenuation_coeff")) {
			constantAttenuation = getField(child, "constant_attenuation_coeff")->getScalar();
		}

		if (hasField(child, "linear_attenuation_coeff")) {
			linearAttenuation = getField(child, "linear_attenuation_coeff")->getScalar();
		}

		if (hasField(child, "quadratic_attenuation_coeff")) {
			quadricAttenuation = getField(child, "quadratic_attenuation_coeff")->getScalar();
		}

		scene->add(new PointLight(scene,
			tupleToVec(getField(child, "position")),
			tupleToVec(getColorField(child)), constantAttenuation, linearAttenuation,quadricAttenuation));
	}
