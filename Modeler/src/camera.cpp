	...
	case kActionTranslate:
		{
			...
			double xTrack =  -mouseDelta[0] * kMouseTranslationXSensitivity;   //正负号注意
			double yTrack =  mouseDelta[1] * kMouseTranslationYSensitivity;

			Vec3f transXAxis = mUpVector ^ (mPosition - mLookAt);  //??????? 叉乘
			transXAxis /= sqrt((transXAxis*transXAxis));  //归一化
